...
class ComponentRelation extends AbstractComponent
{
    ...
    {
        $modelName = $this->assetModel->model;
        $model = $modelName::find($this->recordId);
        $relation = $this->properties()->relation;
    ...
    private function getChildTable()
    {
        ...
        $model = new $modelName;
        $relation = $this->properties()->relation;
        $r = new \ReflectionClass($model->$relation()->getRelated());
        $childModel = $r->getName();
        $childTable = new $childModel;
        $childTable = $childTable->getTable();
    ...
    public function setValueFromStorage(mixed $value): bool
    {
        ...
        $table = Table::whereTable($this->getChildTable())->first();
    ...
    protected function customProperties(): object
    {
        ...
        return (object) ['allowedBlocks' => $this->getChildTable()];
    }
    ...
    {
        // dd($this->properties()->childTable,$this->getForeignKey());
        return DB::table($this->getChildTable())->where($this->getForeignKey(), $this->recordId)->where('deleted_at', null)->orderBy('order')->get();
    }
